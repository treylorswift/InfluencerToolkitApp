...
import {TwitterFollower, TwitterFollowerCache} from './TwitterUser';
...
//need to resume)
type FollowerTaskProgress = {cursor:string, lastStoredId:string, startTime:Date, finishTime:Date}
...
export class TwitterDB
{
    ...
    Init():boolean
    {
        this.db = new DB('foobar.db');//, { verbose: console.log });
        ...
        try
        {
            ...
            let createFollowerTasks = this.db.prepare(`
                CREATE TABLE IF NOT EXISTS TwitterFollowerTasks (
                    id_str TEXT NOT NULL UNIQUE PRIMARY KEY,
                    cursor TEXT DEFAULT "",
                    last_stored_follower TEXT DEFAULT "",
...
abstract class TwitterFollowerCacheBase
{
    //@ts-ignore
    ...
    abstract GetPriorProgress():FollowerTaskProgress

    abstract GetNumStoredFollowers():number;

    abstract BeginWriting();
    abstract WriteFollowers(followers:Array<{follower:any, dbIndex:number}>, progress:FollowerTaskProgress); //f is what comes right out of the twitter api pipe
 
    //returns true if the cache has not been built or is only partially built
    //returns false if the cache is completely built / finished
    NeedToBuildFollowerCache():boolean
    {
        let p = this.GetPriorProgress();
            ...
            while (1)
            {
                ...
                while (numProcessed<totalToProcess)
                {
                    ...
                    for (var i=0; i<usersResult.length; i++)
                    {
                        ...
                        //WriteFollowers will save this progress in a separate table
                        //so we can resume later if we dont finish / get aborted
                        let progress:FollowerTaskProgress = {
                            cursor:cursor,
                            lastStoredId:usersResult[i].id_str,
                            startTime:startTime,
                            finishTime:null
                        }

                        //check if we're on the last write and if so, include a finish time in the progress
                        if (i===usersResult.length-1 &&
                            idsResult.next_cursor_str==="0")
                        {
                            progress.finishTime = new Date();
                        }

                        await this.WriteFollowers(followersToWrite, progress);
                    }
...
export class TwitterFollowerCacheSQL extends TwitterFollowerCacheBase
{
    ...
    private StoreProgress(progress:FollowerTaskProgress)
    {
        try
        {
            //update the task table with our progress
            let updateTaskTable = this.db.prepare('REPLACE INTO TwitterFollowerTasks (id_str,cursor,last_stored_follower,start_time,finish_time) VALUES (?,?,?,?,?)');
            ...
            let lastStoredId = progress.lastStoredId ? progress.lastStoredId : '';
            ...
            updateTaskTable.run([this.id_str, cursor, lastStoredId, startTime, finishTime]);
        }
        catch (err)
        {
    ...
    GetPriorProgress():FollowerTaskProgress
    {
        try
        {
            ...
            let p:FollowerTaskProgress = {
                ...
                lastStoredId:result.last_stored_follower,
                startTime:null,
                finishTime:null
            };
        ...
        {
            console.log("Error in GetPriorProgress:");
            console.error(err);
            return null;
        }
    }
    ...
    async WriteFollowers(followers:Array<{follower:any, dbIndex:any}>, progress:FollowerTaskProgress):Promise<boolean>
    {
        var str = '';
            ...
            let innerTransactions = () =>
            {
                for (var i=0; i<followers.length; i++)
                {
                   ...
                   if (insertTags)
                   {
                    ...
                    //now update the task table with our progress
